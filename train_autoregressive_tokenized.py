...
def create_data_loaders(tokenizer, config):
    ...
    # Load tokenized data from create_tokenized_data.py output
    data_path = Path(config['data']['data_dir']) / "data"
    
    def load_tokenized_data(split):
        # Load source (NMR tokens) and target (SMILES) from text files
        with open(data_path / f"src-{split}.txt") as f:
            sources = [line.strip() for line in f]
        with open(data_path / f"tgt-{split}.txt") as f:
            targets = [line.strip() for line in f]
            
        # Load IR data from parquet/numpy files if they exist
        ir_data_path = data_path / f"ir-{split}.npy"
        ir_data = None
        if ir_data_path.exists():
            ir_data = np.load(ir_data_path)
            
        return sources, targets, ir_data

    # Load all splits
    train_src, train_tgt, train_ir = load_tokenized_data("train")
    val_src, val_tgt, val_ir = load_tokenized_data("val")
    test_src, test_tgt, test_ir = load_tokenized_data("test")

    print(f"[DataLoader] Loaded {len(train_src)} training samples")
