...
class SMILESDiffusionDecoder(nn.Module):
    """
    LLADA-style mask predictor over SMILES tokens.

    This keeps the same prompt-conditioning interface as the autoregressive
    decoder, but removes causal masking so every target position can attend to
    every other target position while still cross-attending to spectral prompt
    memory.
    """
    ...
    def _build_block_diffusion_mask(
        self,
        tgt_len: int,
        block_start: int,
        block_end: int,
        ...
        if not (0 <= block_start < block_end <= tgt_len):
            raise ValueError(
                f"Invalid block range [{block_start}, {block_end}) for target length {tgt_len}."
            )
        ...
        **_: dict,
        ...
        if block_start is not None or block_end is not None:
            if block_start is None or block_end is None:
                raise ValueError("block_start and block_end must both be set for block diffusion masking.")
            tgt_mask = self._build_block_diffusion_mask(
                tgt_len=tgt_len,
        ...
        return self.out(self.final_norm(hidden))
